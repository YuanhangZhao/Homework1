...
ib.connect('127.0.0.1',port=port,clientId=master_client_id)
...
while True:
    ...
    if 'trade_order.p' in listdir():
        ...
        trade_order1 = pickle.load(f1)

        contract = Forex(trade_order1['trade_currency'])
        order = MarketOrder(trade_order1['action'],trade_order1['trade_amt'])
